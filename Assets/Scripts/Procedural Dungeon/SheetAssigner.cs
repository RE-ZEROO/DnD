    ...
    public void Assign(Room[,] rooms)
    {
        foreach(Room room in rooms)
        {
            ...
            if(room.type == RoomType.START)
            {
                ...
                startRoom.Setup(startSheet, room.gridPos, room.type, room.doorTop, room.doorBottom, room.doorLeft, room.doorRight, room.roomId);
            ...
            if (room.type == RoomType.END)
            {
                ...
                endRoom.Setup(sheetsEnd[_index], room.gridPos, room.type, room.doorTop, room.doorBottom, room.doorLeft, room.doorRight, room.roomId);
            ...
            currentRoom.Setup(sheetsNormal[index], room.gridPos, room.type, room.doorTop, room.doorBottom, room.doorLeft, room.doorRight, room.roomId);
        }
    }
}
